  ...
  BLEPeripheral(req, rdy, rst)
{
  ...
  addAttribute(this->_rx2Characteristic);
  addAttribute(this->_rx2NameDescriptor);
  this->_rx2Characteristic.setEventHandler(BLEWritten, BLESerial::_received2);
...
void BLESerial::end() {
  ...
  this->_rx2Characteristic.setEventHandler(BLEWritten, NULL);
  this->_rxHead = this->_rxTail = 0;
  this->_rx2Head = this->_rx2Tail = 0;
  flush();
...
int BLESerial::available2(void) {
  BLEPeripheral::poll();
  int retval = (this->_rx2Head - this->_rx2Tail + sizeof(this->_rx2Buffer)) % sizeof(this->_rx2Buffer);
  #ifdef BLE_SERIAL_DEBUG
    Serial.print(F("BLESerial::available2() = "));
    Serial.println(retval);
  #endif
  return retval;
}
...
int BLESerial::read2(void) {
  BLEPeripheral::poll();
  if (this->_rx2Tail == this->_rx2Head) return -1;
  this->_rx2Tail = (this->_rx2Tail + 1) % sizeof(this->_rx2Buffer);
  uint8_t byte = this->_rx2Buffer[this->_rx2Tail];
  #ifdef BLE_SERIAL_DEBUG
    Serial.print(F("BLESerial::read2() = "));
    Serial.print((char) byte);
    Serial.print(F(" 0x"));
    Serial.println(byte, HEX);
  #endif
...
void BLESerial::_received2(const uint8_t* data, size_t size) {
  for (size_t i = 0; i < size; i++) {
    this->_rx2Head = (this->_rx2Head + 1) % sizeof(this->_rx2Buffer);
    this->_rx2Buffer[this->_rx2Head] = data[i];
  }
  #ifdef BLE_SERIAL_DEBUG
    Serial.print(F("BLESerial::received2("));
    for (int i = 0; i < size; i++) Serial.print((char) data[i]);
    Serial.println(F(")"));
  #endif
}
...
void BLESerial::_received2(BLECentral& /*central*/, BLECharacteristic& rxCharacteristic) {
  BLESerial::_instance->_received2(rxCharacteristic.value(), rxCharacteristic.valueLength());
}
